...
Imagecraft has a community from all over the world, and the Imagecraft team welcomes all
contributions.

Contributing offers an opportunity to polish your technical skills, develop as a
professional, and get involved in the growing open source community. Imagecraft
...
The Starcraft team at Canonical sets the direction and priorities of Imagecraft. They
take responsibility for its stewardship and health.

Before you start work on Imagecraft, there are three documents for you to digest.
...
Imagecraft is licensed under [LGPL-3.0](LICENSE).

## Report an issue or open a request

If you find a bug or feature gap in Imagecraft, look for it in the [project's GitHub
issues](https://github.com/canonical/imagecraft/issues) first. If you have fresh input,
add your voice to the issue.

If the bug or feature doesn't have an issue, we invite you to [open
one](https://github.com/canonical/imagecraft/issues/new/choose).

## Set up for development

Imagecraft uses a forking, feature-based workflow. Most work on Imagecraft occurs on
...
Start by [creating a personal fork](https://github.com/canonical/imagecraft/fork) of the
...
```bash
git clone git@github.com:<username>/imagecraft --recurse-submodules
cd imagecraft
git remote add upstream git@github.com:canonical/imagecraft
git fetch upstream
```
...
```bash
git clone https://github.com/<username>/imagecraft --recurse-submodules
cd imagecraft
git remote add upstream https://github.com/canonical/imagecraft
git fetch upstream
```
...
All significant work in Imagecraft should be tied to an existing issue or ticket.

Once you find an issue that you'd like to work on, it's best to reach out so that an
Imagecraft maintainer can assign the ticket to you before starting. This process varies
...
one else is working on it, add a comment stating that you'd like to take it on, and an
Imagecraft maintainer will assign it to you.

If you don't find a related issue, [open
one](https://github.com/canonical/imagecraft/issues/new/choose) and indicate that you're
interested in taking it on. When creating the issue, be sure to add any relevant labels.
An Imagecraft maintainer will then review the issue and assign it to you.
...
Imagecraft's test suite includes unit, integration, and
...
the four categories of [Diátaxis](https://diataxis.fr), the framework that Imagecraft's
