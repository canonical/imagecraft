...
import pytest
from imagecraft.snap_config import SnapConfig, get_snap_config
...
def mock_config():
    with patch(
        "imagecraft.snap_config.SnapConfigOptions", autospec=True
    ) as mock_snap_config:
...
def mock_is_running_from_snap(mocker):
    return mocker.patch(
        "imagecraft.snap_config.is_imagecraft_running_from_snap", return_value=True
    )
...
def test_unmarshal_not_a_dictionary():
    ...
    with pytest.raises(TypeError, match=error):
        SnapConfig.unmarshal("provider=lxd")  # type: ignore[reportArgumentType]
...
def test_get_snap_config_not_from_snap(mock_is_running_from_snap):
    """Verify None is returned when imagecraft is not running from a snap."""
