...
``apt``, ``required``, ``minbase``, ``buildd``, ``important``, ``debootstrap``,
...
3. Removes default source lists (``/etc/apt/sources.list`` and ``/etc/apt/souces.list.d/*``)
   to allow for custom repository configuration.
