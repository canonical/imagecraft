...
`canonical/imagecraft` is a CLI tool written in Python for creating Ubuntu bootable
images. It follows the same principles as Snapcraft, but is focused on creating bootable
images instead.

## Craft apps and libraries

Imagecraft is built on the following craft libraries:
...
These libraries are also used by other craft apps, including but not limited to
Charmcraft, Debcraft, Rockcraft, and Snapcraft. The source code for these apps is at
...
Imagecraft uses [uv](https://docs.astral.sh/uv/) for dependency management.
