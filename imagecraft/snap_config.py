...
from imagecraft.utils import is_imagecraft_running_from_snap
...
def get_snap_config() -> SnapConfig | None:
    ...
    """
    if not is_imagecraft_running_from_snap():
        emit.debug(
            "Not loading snap config because imagecraft is not running as a snap."
        )
        return None
